...
const FROMWEB = config.FROMWEB ;

const PORT = config.API_PORT ;
const AnalyticsApp = express();
// Middleware to parse JSON bodies
AnalyticsApp.use(express.json());
AnalyticsApp.use(compression());
AnalyticsApp.use(helmet());
AnalyticsApp.use(cors());

AnalyticsApp.use(bodyParser.urlencoded({ limit: '300mb', extended: true }));
AnalyticsApp.use(bodyParser.json({ limit: '300mb' }));  
...
AnalyticsApp.use('/api/user', userRoute);
AnalyticsApp.use('/api/dataset', datasetRoute); 
AnalyticsApp.use('/api/widget', widgetRoute); 
AnalyticsApp.use('/api/report', reportRoute); 


AnalyticsApp.listen(PORT, () => {
