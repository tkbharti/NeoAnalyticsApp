...
const LoginPage = () => { 
 	...
 	const handleSubmit = async (e) => {
		...
		try{ 
			...
			if (response.data.length>0) { 
				...
				localStorage.setItem('topnav','datasetlist'); 
  ...
  //  backgroundsize: contain;    /* Fits the entire image inside the div */
 // background-repeat: no-repeat; /* Prevents the image from tiling */
 // background-position: center;  /* Centers the image within the div */
  
  /* Ensure your div has dimensions, or it won't show up */
  //width: 100%;
  //height: 400px; 
  return (  
		
		<div className={`login-container bg-${theme.color}`} style={{
		...
		<div className={`txt-${theme.color}`}>
		  <div style={{padding:'10px', color:'#1255c4'}}>Powered By : tarunverse.tech</div>
