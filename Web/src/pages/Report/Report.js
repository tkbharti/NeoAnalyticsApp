...
const Report = () => { 
    ...
    // Initialize Grid
    useEffect(() => {
        if (!gridRef.current && gridEl.current) {
    ...
    // Manual Save
    const saveReport = () => { 
         setState((prev)=>({...prev,  savevisible: true, modalInput:''}));  
    };
    ...
    const handleSaveReport = async () => {  
        if (gridRef.current) {
            ...
            const response = await ReportService.saveReport({metadata: updatedArray, reportname:state.modalInput});
            setState((prev)=>({...prev,  savevisible: false, modalInput:''}));  
        } 
    };
